                ...
                <div class="relative px-6 py-6 bg-gradient-to-r from-emerald-500/10 via-purple-500/5 to-transparent border-b border-gray-100 dark:border-gray-700/60">
                    <div class="flex items-center space-x-3 rtl:space-x-reverse">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Fill in the details below to write and publish a new blog post.</p>
                        </div>
                    </div>
                </div>

                <!-- Form Body -->
                <form action="{{ route('post.store') }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-6">
                    @csrf

                    <!-- Field: Post Title -->
                    <div class="space-y-2">
                        <x-input-label for="title" :value="'Post Title'" class="text-sm font-semibold text-gray-700 dark:text-gray-300" />
                        <x-text-input
                            id="title"
                            name="title"
                            type="text"
                    ...
                    <!-- Field: Post Content -->
                    <div class="space-y-2">
                        <x-input-label for="content" :value="'Post Content'" class="text-sm font-semibold text-gray-700 dark:text-gray-300" />
                        <textarea
                            id="content"
                            name="content"
                            rows="6"
                    ...
                    <div class="space-y-2">
                        <x-input-label for="image" :value="'Featured Image'" class="text-sm font-semibold text-gray-700 dark:text-gray-300" />
                        ...
                        <div x-data="{
                            ...
                            <label for="image"
                                ...
                                <div x-show="!previewUrl" class="flex flex-col items-center justify-center px-4 text-center">
                                    <div class="p-3 mb-3 bg-white dark:bg-gray-800 rounded-xl shadow-sm text-gray-400 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">
                                ...
                                <div x-show="previewUrl"
                                    ...
                                    <span class="text-xs text-emerald-300 hover:text-emerald-200 font-semibold underline">Change Image</span>
                                </div>

                                <input id="image" name="image" type="file" accept="image/*" class="sr-only" @change="handleFileChange($event)">
                            </label>
                        </div>
