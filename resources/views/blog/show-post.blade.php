<x-master-layout>
    <div dir="rtl" class="min-h-[calc(100vh-80px)] py-12 px-4 sm:px-6 lg:px-8 bg-slate-950 text-slate-100 flex justify-center">

        <!-- حاوية المقال الرئيسية -->
        <article class="max-w-4xl w-full bg-slate-900/80 border border-slate-700/70 rounded-3xl p-6 sm:p-10 shadow-xl backdrop-blur-sm">

            <!-- زر العودة للرئيسية أو المدونة -->
            <div class="mb-8">
                <a href="{{ route('blog.main') }}" class="inline-flex items-center gap-2 text-xs sm:text-sm font-medium text-yellow-400 hover:text-yellow-300 transition-colors bg-slate-800/80 px-4 py-2 rounded-full border border-slate-700">
                    &larr; العودة إلى المدونة
                </a>
            </div>

            <!-- معلومات المقال (التاريخ / التصنيف) -->
            <div class="flex items-center justify-between flex-wrap gap-4 mb-6 text-xs sm:text-sm text-slate-400 border-b border-slate-800 pb-4">
                <div class="flex items-center gap-2">
                    <span class="px-3 py-1 rounded-md bg-yellow-500/10 text-yellow-400 font-medium border border-yellow-500/20">
            ...
            @if($post->image)
                <div class="mb-10 rounded-2xl overflow-hidden border border-slate-800 shadow-lg bg-slate-800 max-h-[500px] flex items-center justify-center">
