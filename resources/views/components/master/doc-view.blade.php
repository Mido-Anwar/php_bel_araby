<div dir="rtl"
  ...
  <aside
    ...
    {{-- ═══════════════════════════════════════════════ --}}
    {{-- 1. المفاهيم (Concepts) — فوق --}}
    {{-- ═══════════════════════════════════════════════ --}}
    <div class="mb-8">
        <h3 class="text-sm font-bold text-yellow-400 mb-4 flex items-center gap-2">
            <span class="w-1 h-4 rounded-full bg-yellow-400"></span>
            المفاهيم (Concepts)
        </h3>
        ...
        <div class="space-y-4">
            @foreach ($technology->sections as $section)
                ...
                @if ($concepts->count() > 0)
                    <div class="space-y-2" x-data="{ secId: 'concept-sec-{{ $section->id }}' }">
                        ...
                        <div :id="secId" x-show="openSections[secId]" x-cloak
                            ...
                            class="space-y-1 pr-3 border-r-2 border-slate-800 mr-2">
        ...
        <div class="space-y-4">
            @foreach ($technology->sections as $section)
                ...
                @if ($functions->count() > 0)
                    <div class="space-y-2" x-data="{ secId: 'function-sec-{{ $section->id }}' }">
                        <button @click="toggleSection(secId)"
                            class="w-full flex items-center justify-between px-3 py-2.5 rounded-xl bg-[#1e293b] text-[#f3ebeb] border border-slate-700/50 hover:bg-[#2b384f] hover:text-blue-400 transition-all duration-300 shadow-md">
                            <span class="flex items-center gap-2">
                                <span class="w-1.5 h-1.5 rounded-full bg-blue-400"></span>
                                {{ $section->title }}
                            </span>
                            <svg class="w-4 h-4 text-slate-400 transform transform transition-transform duration-300"
                                :class="openSections[secId] ? 'rotate-180' : ''" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M19 9l-7 7-7-7" />
